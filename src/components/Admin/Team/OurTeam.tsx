...
const TeamMembers = () => {
      ...
      if (selectedMemberId !== null) {
        try {
          ...
          const updatedCertificates = await Promise.all(
            certData.map(async (certificate: Certificate) => {
  ...
  const handleDeleteCertificate = async (
    ...
    const response = await fetch(
      `/api/team?id=${teamMemberId}&certificateId=${certificateId}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleCertificateFilesChange = (
  ...
  const columns = [
    ...
    { key: "title", header: "Position" },
    {
      key: "image",
      header: "Image",
      renderCell: (row: TeamMember) => (
        <div className="mx-auto">
          <a data-lightbox="gallery" data-title={row.title} href={row.imageUrl}>
            <img
              src={row.imageUrl}
              alt={row.title}
